...
sudo DEBIAN_FRONTEND=noninteractive apt-get -qq install -y --no-install-recommends \
  ...
  gawk gcc-multilib gettext git gperf help2man intltool libelf-dev libffi-dev \
  ...
  p7zip-full patch pigz pkgconf python3 python3-dev python3-pip \
  python3-pyelftools python3-setuptools qemu-utils rsync scons squashfs-tools \
  subversion swig texinfo uglifyjs unzip vim-common wget xmlto xsltproc \
  zlib1g-dev zstd genisoimage
