...
- `arm-flash`: ARM rootfs plus the automatically packaged N1 flash image.
...
ccache. Every restored host/toolchain cache is checked by running the matching
OpenWrt make target before the firmware build continues. Only the compressed
firmware bundle is retained as a workflow artifact; individual files are
